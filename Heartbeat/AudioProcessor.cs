...
namespace Heartbeat {
    ...
    internal sealed class AudioProcessor : VstPluginAudioProcessor, IVstPluginBypass {
        ...
        public AudioProcessor(IVstPluginEvents pluginEvents, PluginParameters parameters)
            ...
            Left = new Heart(parameters.HeartParameters);
            Right = new Heart(parameters.HeartParameters);

            pluginEvents.Opened += Plugin_Opened;
        }

        internal Heart Left { get; private set; }
        internal Heart Right { get; private set; }
        ...
        // calculate how far into the current quarter note we are
        private float GetBeatProgress((float, float) beatParams) {
            return (float)(beatParams.Item2 * (TimeInfo.PpqPosition / beatParams.Item1) % 1f);
        }
